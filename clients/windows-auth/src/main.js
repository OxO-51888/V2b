const { app, BrowserWindow, dialog, ipcMain, screen, Tray, nativeImage, shell, session } = require('electron');
...
const { AuthorizationManager, normalizeMessage } = require('./main/authorization-manager');
...
let authorizationFailureShown = false;
...
async function quitApp() {
  ...
  if (authorization) authorization.stop();
...
function ensureCore() {
  if (!core) {
    ...
    core.setAuthorizationGuard(() => Boolean(authorization?.isAuthorized()));
    core.setAuthorizationEnvironmentProvider(() => authorization.getCoreEnvironment());
  }
  return core;
}

async function showAuthorizationFailureAndQuit(error) {
  if (authorizationFailureShown || isQuitting) return;
  authorizationFailureShown = true;
...
async function requireAuthorization() {
  if (!authorization) throw new Error('授权系统未初始化');
  try {
    const sessionInfo = await authorization.ensureAuthorized();
    ensureCore();
    ensureTray();
    return sessionInfo;
  } catch (error) {
    await showAuthorizationFailureAndQuit(error);
    throw error;
  }
}

async function readAuthorizedState() {
  const savedState = await store.read();
  applyAuthorizationIdentity(savedState);
  const sessionInfo = await requireAuthorization();
  return withGatewayState(savedState, sessionInfo);
}

function withGatewayState(state = {}, sessionInfo = authorization?.getSession()) {
  return {
    ...state,
    baseUrl: authorization.getGatewayPanelBaseUrl(),
    gatewaySessionToken: sessionInfo?.sessionToken || ''
  };
}

function applyAuthorizationIdentity(state = {}) {
  if (!authorization) return;
  authorization.setClientContext({
...
async function syncAuthorizationIdentity(userEmail = '') {
  if (!authorization) return;
  const context = { userEmail };
  try {
    const location = await getLocalLocation();
    context.localIp = location.ip || '';
    context.localLocation = location.locationText || '';
    const current = await store.read();
    await store.write({
      ...current,
      userEmail: userEmail || current.userEmail || '',
      localIp: context.localIp,
      localLocation: context.localLocation
    });
  } catch {}
  authorization.setClientContext(context);
  if (authorization.isAuthorized()) {
    await authorization.heartbeat().catch(() => {});
  }
}
...
function dashboardEmail(dashboard) {
  return String(dashboard?.user?.email || dashboard?.user?.email_address || dashboard?.user?.mail || '').trim();
}

async function fetchDashboardAndRemember(state = null) {
  const sessionInfo = await requireAuthorization();
  const currentState = withGatewayState(state || await store.read(), sessionInfo);
  latestDashboard = await panelApi.fetchDashboard(currentState);
  const email = currentState.userEmail || dashboardEmail(latestDashboard);
  if (email) syncAuthorizationIdentity(email).catch(() => {});
  return latestDashboard;
}

async function refreshDashboardForModeSwitch() {
  const state = await readAuthorizedState();
...
app.whenReady().then(async () => {
  ...
  authorization = new AuthorizationManager(app, {
    onFailure: showAuthorizationFailureAndQuit
  });
  createWindow();
});
...
ipcMain.handle('auth:login', async (_event, credentials) => {
  ...
  authorization.setClientContext({ userEmail: email });
  const sessionInfo = await requireAuthorization();
  const state = await panelApi.login({
    baseUrl: authorization.getGatewayPanelBaseUrl(),
    sessionToken: sessionInfo.sessionToken
  }, email, String(credentials?.password || ''));
  const previous = await store.read();
  await store.write({ ...previous, auth: state.auth, userEmail: email });
  syncAuthorizationIdentity(email).catch(() => {});
  return { authenticated: true, dashboard: await fetchDashboardAndRemember({ ...state, userEmail: email }) };
});
...
ipcMain.handle('dashboard:refresh', async () => {
  return fetchDashboardAndRemember(await readAuthorizedState());
});
...
ipcMain.handle('core:disconnect', async () => {
  await setWindowsProxy(false).catch(() => {});
  const result = core ? await core.stop() : { running: false };
  notifyTrayState();
  return result;
});
...
ipcMain.handle('order:create', async (_event, payload) => {
  return panelApi.request(await readAuthorizedState(), '/user/order/save', {
    method: 'POST',
    body: payload
  });
});
...
ipcMain.handle('coupon:check', async (_event, payload) => {
  return panelApi.request(await readAuthorizedState(), '/user/coupon/check', {
    method: 'POST',
    body: payload
  });
});
...
ipcMain.handle('order:detail', async (_event, tradeNo) => {
  return panelApi.request(await readAuthorizedState(), `/user/order/detail?trade_no=${encodeURIComponent(tradeNo)}`);
});

ipcMain.handle('order:paymentMethods', async () => {
  return panelApi.request(await readAuthorizedState(), '/user/order/getPaymentMethod');
});

ipcMain.handle('order:checkout', async (_event, payload) => {
  return panelApi.request(await readAuthorizedState(), '/user/order/checkout', {
    method: 'POST',
    body: payload
  });
});
...
ipcMain.handle('order:check', async (_event, tradeNo) => {
  return panelApi.request(await readAuthorizedState(), `/user/order/check?trade_no=${encodeURIComponent(tradeNo)}`);
});

ipcMain.handle('order:cancel', async (_event, tradeNo) => {
  return panelApi.request(await readAuthorizedState(), '/user/order/cancel', {
    method: 'POST',
