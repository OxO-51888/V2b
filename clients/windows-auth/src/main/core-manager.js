...
const { AUTH_CORE_SHA256 } = require('./app-config');
...
class CoreManager {
  constructor(app, notifyStatus) {
    ...
    this.authorizationEnvironmentProvider = null;
    this.process = null;
    this.lastOutput = [];
  ...
  setAuthorizationGuard(guard) {
    this.authorizationGuard = typeof guard === 'function' ? guard : null;
  }

  setAuthorizationEnvironmentProvider(provider) {
    this.authorizationEnvironmentProvider = typeof provider === 'function' ? provider : null;
  }

  assertAuthorized(action = '内核组件') {
    if (this.authorizationGuard && !this.authorizationGuard()) {
      throw new Error(`${action}未获得授权，禁止运行`);
    }
  }

  getAuthorizationEnvironment() {
    this.assertAuthorized('core authorization');
    if (!this.authorizationEnvironmentProvider) {
      throw new Error('core authorization provider missing');
    }
    const env = this.authorizationEnvironmentProvider();
    if (!env?.XIAOV2B_CORE_LICENSE_PAYLOAD || !env?.XIAOV2B_CORE_LICENSE_SIGNATURE) {
      throw new Error('core authorization payload missing');
    }
    return env;
  }
  ...
  async verifyCoreIntegrity(corePath) {
    const expected = String(AUTH_CORE_SHA256 || '').trim().toLowerCase();
    if (!expected) return;
    const bytes = await fs.readFile(corePath);
    const actual = crypto.createHash('sha256').update(bytes).digest('hex');
    if (actual !== expected) {
      throw new Error('core integrity check failed');
    }
  }
...
`;
  ...
  async start(profilePath) {
    ...
    await this.verifyCoreIntegrity(corePath);
    const coreAuthorizationEnv = this.getAuthorizationEnvironment();
